                  ...
                  if($today == substr($learner[next_lesson_date], 0, 10)){ 
                    $times = downcounter($learner[next_lesson_date]);
                        if($times[0] == 0 && $times[1] == 0 && $times[2] <= 15 && $times[2] >= 0){
                          	getLessonByClientId($learner[id], 1, $learner[custom_individual_chat_id], $learner[next_lesson_date], $learner[custom_zoom_link], $learner[next_lesson_date]);
...
function getLessonByClientId($clientId, $branchId, $individualChatId, $nextLessonDate, $zoomLink, $checkStartLesson){
  	if(getToken() == ""){
      apiAuthorise();
	 getLessonByClientId($clientId, $branchId, $individualChatId, $nextLessonDate, $zoomLink, $checkStartLesson);
    }else{
          $today =  "20".date("y")."-".date("m")."-".date("d");
          $ch = curl_init();
               ...
               getLessonByClientId($clientId, $branchId, $individualChatId, $nextLessonDate, $zoomLink,$checkStartLesson);
                ...
                foreach($result as $lesson){
                  if($lesson[type] == 1 || $lesson[type] == 2){
                	if($lesson[status] == 1){
                      if($lesson[start] == $checkStartLesson){
                          //print_r($lesson);
                          if($lesson[type] == 1){//индивидуалный
                            ...
                            //echo $chatIdGroup." asdddddd ";
                               if($chatIdGroup != "bad" && $chatIdGroup != null){
                                 	if(check15MinReminder($today, $chatIdGroup) == "nice"){
                                      
                                        lessonReminder($chatIdGroup, $nextLessonDate); 
                                          add15MinReminder($today, $chatIdGroup);
