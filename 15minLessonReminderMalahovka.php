...
function getCustomer(){
	  ...
	  getCustomer();
        ...
        for(;;){
          ...
          curl_setopt($ch, CURLOPT_URL, 'https://iecenglish.s20.online/v2api/3/customer/index');
          curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
                  ...
                  if($today == substr($learner[next_lesson_date], 0, 10)){ 
                    $times = downcounter($learner[next_lesson_date]);
                        if($times[0] == 0 && $times[1] == 0 && $times[2] <= 15 && $times[2] >= 0){
                          	getLessonByClientId($learner[id], 3, $learner[custom_individual_chat_id], $learner[next_lesson_date], $learner[custom_zoom_link]);
