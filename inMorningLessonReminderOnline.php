              ...
              if($code == "403" || $code == "401"){
                ...
                foreach($result[items] as $learner) {
                    if($today == substr($learner[next_lesson_date], 0, 10)){ 
                      foreach($learner[branch_ids] as $brnch){
                      getLessonByClientId($learner[id], $brnch, $learner[custom_individual_chat_id], $learner[next_lesson_date], $learner[custom_zoom_link]);
                      }
                    }
                }
                }
                          ...
                          if($lesson[type] == 1){//индивидуалный
                            ...
                            //echo $chatIdGroup." asdddddd ";
                               if($chatIdGroup != "bad" && $chatIdGroup != null){
                                 	if(checkMorningReminder($today, $pieces[0]) == "nice"){
                                      if($branchId == 5){
                                        lessonReminderOnline($pieces[0], $nextLessonDate, $pieces[1]); 
                                        addMorningReminder($today, $pieces[0]);
                                      }else{
                                        lessonReminder($pieces[0], $nextLessonDate); 
                                        addMorningReminder($today, $pieces[0]);
                                      }
                                    }
                               }
